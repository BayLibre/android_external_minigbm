...
/* A dumb buffer is CPU-visible linear memory with no gbm_bo behind it, so it is
 * only safe for buffers the GPU never touches. HAL_PIXEL_FORMAT_BLOB is the case
 * that matters here: it arrives as DRM_FORMAT_R8 with height 1 and a width equal
 * to the whole payload (5 MB for a camera JPEG), which no GBM surface can
 * represent, and it is only ever written and read by the CPU. */
static bool needs_no_gpu_access(uint64_t use_flags)
{
	return !(use_flags & (BO_USE_TEXTURE | BO_USE_RENDERING | BO_USE_SCANOUT));
}
...
struct GbmMesaDriver {
	...
	/*
	 * KMS card node kept open only for drm_mode_create_dumb. Needed on SoCs
	 * whose GPU has a separate display controller but which are *not* in
	 * separate_dc_gpu_list, i.e. where Mesa cannot create a gbm device on the
	 * KMS node (no kmsro pairing) and therefore has to stay on the render
	 * node. There gbm_node_fd is the render node, which has no dumb ioctl.
	 */
	UniqueFd dumb_node_fd;
								  ...
								  bool mapper_sphal)
{
	...
	if (!drv->priv) {
		...
		if (look_for_kms && !mapper_sphal) {
			...
			/*
			 * The GPU node has no drm_mode_create_dumb, so the YUV
			 * fallback in gbm_mesa_alloc_dumb() has nothing to allocate
			 * from. Keep a KMS card node open just for that. This is
			 * deliberately not done by putting the GPU in
			 * separate_dc_gpu_list: that would also move the Mesa gbm
			 * device onto the KMS node, which only works where Mesa has a
			 * kmsro pairing for that display driver.
			 */
			if (!mapper_sphal) {
				open_drm_dev(true, [&](int fd, bool is_kms,
						       std::string drm_name) -> bool {
					if (!is_kms || gbm_mesa_drv->dumb_node_fd)
						return false;

					gbm_mesa_drv->dumb_node_fd = UniqueFd(fd);
					drv_logi("Found KMS dev %s for dumb buffers\n",
						 drm_name.c_str());
					return true;
				});
				/* Never hold master, the composer needs it. */
				if (gbm_mesa_drv->dumb_node_fd)
					drmDropMaster(gbm_mesa_drv->dumb_node_fd.Get());
			}
		}
			       ...
			       std::shared_ptr<GbmMesaDriver> gbm_drv)
{
	 ...
	 * The GPU fd (Panfrost, PowerVR) doesn't support dumb buffer ioctls,
	 * but the KMS fd (meson-drm, spacemit-drm) does. On KMSRO systems
	 * gbm_node_fd already is the KMS node; elsewhere it is the render node
	 * and dumb_node_fd holds a card node opened just for this. */
	int fd = gbm_drv->dumb_node_fd ? gbm_drv->dumb_node_fd.Get() : gbm_drv->gbm_node_fd.Get();
	if (fd < 0) {
	...
	drv_logv("Allocated dumb buffer: %ux%u format=0x%x size=%zu",
		       ...
		       uint64_t use_flags)
{
	...
	/* Formats GBM cannot represent (NV12 and friends) go straight to a dumb
	 * buffer, as long as the GPU is not going to touch them. */
	if (wr->get_gbm_format(format) == 0 &&
	    (is_yuv_format(format) || needs_no_gpu_access(use_flags))) {
	...
	/* Fail early for unsupported formats the GPU does need */
	if (wr->get_gbm_format(format) == 0) {
		drv_loge("Format 0x%x not supported by GBM and needs GPU access "
			 "(use_flags=0x%llx)",
			 format, (unsigned long long)use_flags);
		return -EINVAL;
	}
	...
	if (err) {
		/* Fall back to a dumb buffer only when the GPU will not import this
		 * as a texture or render target -- YUV camera/video planes, and
		 * CPU-only buffers such as BLOB. Anything GPU-visible must fail
		 * loudly instead of getting memory it cannot use. */
		if (is_yuv_format(format) || needs_no_gpu_access(use_flags)) {
			drv_loge("GBM alloc failed, falling back to dumb buffer "
				 "(format=0x%x use_flags=0x%llx)",
				 format, (unsigned long long)use_flags);
			return gbm_mesa_alloc_dumb(bo, width, height, format, use_flags, drv);
		}
